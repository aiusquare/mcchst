...
const AccountClearance = () => {
  ...
  const [a4Issued, setA4Issued] = useState("");
  const [foolscapIssued, setFoolscapIssued] = useState("");
  ...
  const fetchRecord = async (targetId = identifier) => {
    ...
    try {
      ...
      const res = await request
        ...
        .type("application/json")
        .send({ email: targetId, role: "accounting" });

      const data = res.body?.data || null;
      setRecord(data);
      setStudent(res.body?.student || null);
      // Prefer new AOA4/AOFoolscaps fields, fall back to legacy BS* if present
      setA4Issued(data?.AOA4 ?? data?.BSOutstandingFees ?? "");
      setFoolscapIssued(data?.AOFoolscaps ?? data?.BSOtherLiabilities ?? "");
    ...
    try {
      ...
      await request
        ...
        .send({
          email: identifier,
          role: "accounting",
          // Write to both new and legacy fields to keep data aligned
          AOA4: a4Issued,
          AOFoolscaps: foolscapIssued,
          BSOutstandingFees: a4Issued,
          BSOtherLiabilities: foolscapIssued,
        });
  ...
  return (
    <div className="container py-4">
      <h5 className="mb-3">Accounts Clearance</h5>
            ...
            <div className="row g-3">
              <div className="col-md-6 d-flex align-items-center">
                <input
                  ...
                  checked={a4Issued === "yes"}
                  onChange={(e) => setA4Issued(e.target.checked ? "yes" : "no")}
                />
                <label htmlFor="a4-issued" className="form-check-label">
                  A4 Issued (save as yes/no)
                </label>
              </div>
              <div className="col-md-6 d-flex align-items-center">
                <input
                  ...
                  type="checkbox"
                  className="form-check-input me-2"
                  checked={foolscapIssued === "yes"}
                  onChange={(e) =>
                    setFoolscapIssued(e.target.checked ? "yes" : "no")
                  }
                />
                <label htmlFor="foolscap-issued" className="form-check-label">
                  Foolscap Issued (save as yes/no)
                </label>
              </div>
            </div>
            <button
